...
The Java suite is **528 tests**, of which 42 are the Fixtures — 41 ported from
the reference corpus plus `features/break-inside`, which is this port's own
because the reference engine implements no break properties at all. It is larger than
